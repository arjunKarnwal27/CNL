...
overlaytest = false;
if(overlaytest){
makeLine(1744,1304,2096,1128,2448,1104,2592,832,3064,728,3784,976,4368,1640,4344,3056,4344,3104);//run("Clear", "slice"); // to delete stuff - prob makeselection before
}
...
if(test10){
	...
	rename(originalName);
	Dialog.create("Sample");
	Dialog.addNumber("num here: ", 17);
	Dialog.show();
	e = Dialog.getNumber();
	
	redGreenName = "with_red/Green " + name;
	
	run("Duplicate...", "title=with_Red/Green duplicate");
	rename(redGreenName);
	run("Red/Green");
	imageCalculator("Add create stack", originalName, redGreenName);
	rename("addedImage for " + name);
	
	run("Kuwahara Filter", "sampling=e");
	run("blue orange icb");
	rename("blue/orange " + name);
	run("Duplicate...", "title=Binary duplicate");
	run("Make Binary", "calculate");
	rename("Binary " + name);
	
	run("Stack to Images");
	selectImage("Binary-0002");
	run("Remove Outliers...", "radius=13 threshold=50 which=Bright");
	
	selectImage("Binary-0003");

	// sweep start
	
	// sweep end
	// subtract those found in binary-0003 [with limits, soon]
	// they are the RPE mainly
...
if(test9){
	...
	run("Red/Green");
	print("here2");
	//imageCalculator("Add create stack", "name","name2");
	//selectImage("Result of 5621.svs - Series 2");
	//run("Duplicate...", "title = name2 duplicate");
...
selectImage("right");
makeRectangle(midPT, 0, width-midPT, height);
...
name = outputFolder + File.separator + "right" + list[image] ; 
saveAs("Tiff", name);
...
name = outputFolder + File.separator + "left" + list[image]; 
saveAs("Tiff", name);
...
function getControlPointBackwards(xPts,yPts) { 
// function description
foundFirst = false;

for (b = xPts.length-2; b >= 0; b--) {
yR = round(yPts[b]);
v = getPixel(xPts[b],yR);
redf = (v>>16)&0xff; 
yRn = round(yPts[b+1]);
...
function getControlPointModified(xPts,yPts) { 
// function description
foundFirst = false;
for (b = 0; b < xPts.length - 1; b++) {
yR = round(yPts[b]);
...
function getControlPointModified2(xPts,yPts) { 
// function description
foundFirst = false;
for (b = xPts.length-1; b >=0; b--) {
yR = round(yPts[b]);
v = getPixel(xPts[b],yR);
//yRn = round(yPts[b+1]);
//vNext = getPixel(xPts[b+1],yRn);
...
xPts = newArray(width-cenX);
	...
	//makeSelection("freeline", xPts, yPts);
	pt = getControlPointModified2(xPts,yPts);
	x = pt[0];
	y = pt[1];
		}
		else{
	...
	//makeSelection("freeline", xPts, yPts);
	pt = getControlPointModified(xPts,yPts);
	x = pt[0];
	y = pt[1];
	// can also just straighten 2x 
	...
	cleanXList1 = Array.deleteValue(fListX, 500);
	cleanYList1 = Array.deleteValue(fListY, 500);
	
	cleanXList1 = Array.deleteValue(cleanXList1, 250);
	cleanYList1 = Array.deleteValue(cleanYList1, 250);
	...
	wait(3);
	for (i = 0; i < 0; i++) {
	Fit.doFit(i, cleanXList1, cleanYList1);
	Fit.plot();
	wait(10);
	}
...
xPts = newArray(width-cenX);
	...
	cleanXList1 = Array.deleteValue(fListX, 250); // do negative 
	cleanYList1 = Array.deleteValue(fListY, 250);
